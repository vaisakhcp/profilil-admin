  ...
  useEffect(() => {
    ...
    const fetchProfiles = async () => {
      try {
        ...
        const deleted = data.filter((profile: any) => profile.profile.deleted);

        setFeaturedProfiles(featured.length);
        setSuspendedProfiles(suspended.length);
        setDeletedProfiles(deleted.length);
      } catch (error) {
